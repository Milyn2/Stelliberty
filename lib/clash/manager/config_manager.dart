...
class ConfigManager {
  ...
  String _mode = ClashDefaults.defaultOutboundMode;
  ...
  void _loadPersistedConfig() {
    ...
    _mode = ClashPreferences.instance.getOutboundMode();
  ...
  Future<bool> setTunEnable(bool enabled) async {
    try {
      Logger.debug(
        '🔍 setTunEnable 被调用: enabled=$enabled, 当前 _tunEnable=$_tunEnable, isRunning=${_isRunning()}',
      );
      ...
      Logger.info(
        '虚拟网卡模式配置已更新：${enabled ? "启用" : "禁用"}（${_isRunning() ? "将通过配置热重载生效" : "将在下次启动时生效"}）',
      );
      return true;
    } catch (e) {
  ...
  Future<bool> setMode(String mode) async {
    if (!_isRunning()) {
      Logger.warning('Clash 未运行，无法设置出站模式');
      return false;
    }

    try {
      final success = await _apiClient.setMode(mode);
      if (success) {
        _mode = mode;
        await ClashPreferences.instance.setOutboundMode(mode);
        _notifyListeners();
        Logger.info('出站模式已切换：$mode');
      }
      return success;
    } catch (e) {
  ...
  Future<bool> setModeOffline(String mode) async {
    try {
      ...
      await ClashPreferences.instance.setOutboundMode(mode);
      _notifyListeners();
      Logger.info('出站模式已保存（离线）：$mode，将在下次启动时应用');
      return true;
    } catch (e) {
      Logger.error('保存出站模式失败：$e');
      return false;
    }
  }
